...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^cesium_") ; then
        CESIUM_BUILD=$(echo -n $1 | sed -e 's/^cesium_//g')
    else
        CESIUM_BUILD=
    fi
    export CESIUM_BUILD
...
function lunch()
{
    ...
    then
        # if we can't find a product, try to grab it off the Cesium-Devices GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/cesium/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/cesium/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
